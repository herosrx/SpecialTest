...
public class MailSender {
	...
	private static final String MAIL_SENDER = "special_test@sina.com";
	private static final String MAIL_PWD = "com.cocoatest.st";
	private static final String MAIL_SMTP = "smtp.sina.com";
	 ...
	 * 待发送的邮件的信息
	 */
	public static boolean sendTextMail(String subject, String content, String file, String[] maillists) {
		if (maillists == null || maillists.length == 0 || ("".equals(maillists[0].trim()))) {
			...
			// 如果需要身份认证，则创建一个密码验证器
			authenticator = new CustomizedAuthenticator(MAIL_SENDER, MAIL_PWD);
			...
			try {
				...
				// 创建邮件发送者地址
				Address from = new InternetAddress(MAIL_SENDER);
				// 设置邮件消息的发送者
				mailMessage.setFrom(from);
				// 创建邮件的接收者地址，并设置到邮件消息中
				InternetAddress[] addressTo = new InternetAddress[maillists.length];
